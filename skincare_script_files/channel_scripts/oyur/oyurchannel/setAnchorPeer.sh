...
. ${PWD}/skincare_script_files/channel_scripts/oyur/configUpdate.sh
...
createAnchorPeerUpdate() {
  ...
  infoln "Generating anchor peer update transaction for peer${PEER} of oyur on channel $CHANNEL_NAME"
...
updateAnchorPeer() {
  peer channel update -o orderer0:9056 --ordererTLSHostnameOverride orderer0 -c $CHANNEL_NAME -f ${CORE_PEER_LOCALMSPID}anchors.tx --tls --cafile "$ORDERER_CA" >&log.txt
  res=$?
  cat log.txt
