...
. ${PWD}/skincare_script_files/channel_scripts/lab/configUpdate.sh
...
createAnchorPeerUpdate() {
  ...
  infoln "Generating anchor peer update transaction for peer${PEER} of lab on channel $CHANNEL_NAME"
...
updateAnchorPeer() {
  peer channel update -o orderer1:9057 --ordererTLSHostnameOverride orderer1 -c $CHANNEL_NAME -f ${CORE_PEER_LOCALMSPID}anchors.tx --tls --cafile "$ORDERER_CA" >&log.txt
  res=$?
  cat log.txt
