...
ORGNAME=$2
ORGNAME_ALL_LOWER="${ORGNAME,,}"

CHANNELNAME="oyurchannel"

CC_NAME="skincare"
CC_SRC_PATH="./skincare_cc_files/${ORGNAME_ALL_LOWER}/${CC_NAME}.tgz"
...
CC_COLL_CONFIG="./skincare_privatedata_files/${CHANNELNAME}/collections_config.json"
...
#sudo chmod +x ${PWD}/edi_script_files/channel_scripts/${ORGNAME_ALL_LOWER}/${CHANNELNAME}/package_install_approve_query.sh
#bash ${PWD}/edi_script_files/channel_scripts/${ORGNAME_ALL_LOWER}/${CHANNELNAME}/package_install_approve_query.sh $CC_NAME $CC_SRC_PATH $CC_VERSION $CC_SEQUENCE $CC_COLL_CONFIG

#Here the calculate_store_package_id.sh technically has a lot of extra parameters but we are still keeping them to maintain uniformity

if [ "$1" = "initial" ]; then
   sudo chmod +x ${PWD}/skincare_script_files/channel_scripts/${ORGNAME_ALL_LOWER}/${CHANNELNAME}/calculate_store_package_id.sh
   docker exec cli_${ORGNAME_ALL_LOWER} ./scripts/channel_scripts/${ORGNAME_ALL_LOWER}/${CHANNELNAME}/calculate_store_package_id.sh $CC_NAME $CC_SRC_PATH   $CC_VERSION $CC_SEQUENCE $CC_COLL_CONFIG
elif [ "$1" = "later" ]; then
    sudo chmod +x ${PWD}/skincare_script_files/channel_scripts/${ORGNAME_ALL_LOWER}/${CHANNELNAME}/install_approve_query.sh
docker exec cli_${ORGNAME_ALL_LOWER} ./scripts/channel_scripts/${ORGNAME_ALL_LOWER}/${CHANNELNAME}/install_approve_query.sh $CC_NAME $CC_SRC_PATH $CC_VERSION $CC_SEQUENCE $CC_COLL_CONFIG
else
    echo "Invalid argument. Please provide 'initial' or 'later'."
fi
