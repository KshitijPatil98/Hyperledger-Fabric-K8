...
fetchChannelConfig() {
  ...
  set -x
  peer channel fetch config config_block.pb -o ${ORDERER_NODE_NAME}.${PROJECT_NAME}.com:${ORDERER_NODE_LISPORT} --ordererTLSHostnameOverride ${ORDERER_NODE_NAME}.${PROJECT_NAME}.com -c $CHANNEL --tls --cafile "$ORDERER_CA"
  { set +x; } 2>/dev/null
